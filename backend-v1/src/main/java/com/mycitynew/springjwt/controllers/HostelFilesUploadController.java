...
import com.mycitynew.springjwt.util.HostelUtil;
...
public class HostelFilesUploadController {
    ...
    public ResponseEntity<FileUploadResponse> uploadFiles(@RequestParam("files") MultipartFile[] files) {
        try {
            ...
            Arrays.asList(files).stream().forEach(file -> {
                ...
                try {
                    bytes = file.getBytes();
                    Files.write(Paths.get(HostelUtil.folderPath + file.getOriginalFilename()), bytes);
    ...
    private void createDirIfNotExist() {
        ...
        File directory = new File(HostelUtil.folderPath);
        if (! directory.exists()){
            directory.mkdir();
        }
    }
     ...
     * @return file
     */
    @GetMapping("/hostel/files")
    public ResponseEntity<String[]> getListFiles() {
        return ResponseEntity.status(HttpStatus.OK)
                .body( new File(HostelUtil.folderPath).list());
    }
