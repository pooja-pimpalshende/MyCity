...
import com.mycitynew.springjwt.util.CollegeUtil;
...
public class CollegeFilesUploadController {
    ...
    public ResponseEntity<FileUploadResponse> uploadFiles(@RequestParam("files") MultipartFile[] files) {
        try {
            ...
            Arrays.asList(files).stream().forEach(file -> {
                ...
                try {
                    bytes = file.getBytes();
                    Files.write(Paths.get(CollegeUtil.folderPath + file.getOriginalFilename()), bytes);
    ...
    private void createDirIfNotExist() {
        ...
        File directory = new File(CollegeUtil.folderPath);
        if (! directory.exists()){
            directory.mkdir();
        }
    }
     ...
     * @return file
     */
    @GetMapping("/college/files")
    public ResponseEntity<String[]> getListFiles() {
        return ResponseEntity.status(HttpStatus.OK)
                .body( new File(CollegeUtil.folderPath).list());
    }
